<?php
    ...
    if (isset($_SESSION['admin'])) {
        ...
        $check_sql = "SELECT * FROM librarian WHERE admin_email = '$email'";
        /** @var mysqli $conn */
        $result = mysqli_query($conn, $check_sql);

        if (mysqli_num_rows($result) === 1) {
            $line = mysqli_fetch_assoc($result);
            if (password_verify($password, $line['admin_password'])) {
                $_SESSION['admin'] = $line['admin_id'];
                echo "<script>
                        alert('Login success!');
                    ...
                    <div class="card mb-5">
                        <div class="card-body d-flex flex-column align-items-center">
                            ...
                            <form class="text-center" method="post">
                                ...
                                ?>
                                <p class="text-muted">Don't have an Account?<br><a href="index.php?page=register_admin">Register here</a></p><br></p> 
                            </form>
                        </div>
                    </div>
                </div>
